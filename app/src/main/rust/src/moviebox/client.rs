        ...
        match signed_request_once(base, path_and_query, method).await {
            Ok(resp) => {
                ...
                let body: Value = resp
                    .json()
                    .await
                    .context("MovieBox response was not valid JSON")?;
                ...
                if let Some(code) = body.get("code").and_then(|c| c.as_i64()) {
                    if code != 0 {
                        ...
                        bail!("MovieBox API error (code {code}): {msg}");
                    }
                }
...
async fn ensure_token() -> Result<()> {
    ...
    // if the endpoint itself 4xxs.
    let _ = signed_request_once(HOST_POOL[0], &path, "GET").await;
