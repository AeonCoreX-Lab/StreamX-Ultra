...
//
// Korean drama indexers — ported from Jackett's torrentqq.yml,
// torrentsome.yml, and torrenttip.yml. All three are KOREAN public
// trackers with no login. TorrentTip additionally has explicit
// "한국드라마" (Korean TV/Drama) and Netflix Movies/Series categories,
// which the other two don't break out separately — useful as a stronger
// drama-specific signal when available.
//
// IMPORTANT SITE LIMITATION (verified against all three YAML definitions):
// None of the three expose real seeder/leecher counts — Jackett's own
// definitions hardcode `seeders: {text: 1}` because these sites don't
...
// the way 1337x/TGx/RARBG track them). TorrentTip additionally hardcodes
// a placeholder size (`text: 512MB`) — its real size lives on the detail
...
// "TorrentQQ"/"Torrentsome"/"TorrentTip" as "health unknown", not "low
// seeders" or "unknown size" specifically for TorrentTip.
//
// All three require a detail-page fetch to extract the magnet/infohash —
...
const TORRENTQQ_FALLBACK: &str = "https://torrentqq418.com";
const TORRENTSOME_FALLBACK: &str = "https://torrentsome252.com";
const TORRENTTIP_FALLBACK: &str = "https://torrenttip233.top";
...
pub async fn search_torrentqq(client: &reqwest::Client, mirrors: &[String], query: &str) -> Vec<TorrentResult> {
    let q = urlencoding::encode(query);
    let path = format!("/search?q={q}");
    let mirrors = effective_mirrors(mirrors, TORRENTQQ_FALLBACK);
    fetch_torrentqq_list(client, &mirrors, &path).await.unwrap_or_default()
}
...
async fn fetch_torrentqq_list(client: &reqwest::Client, mirrors: &[String], path: &str) -> Result<Vec<TorrentResult>> {
    ...
    let base_mirror = mirrors.first().cloned().unwrap_or_else(|| TORRENTQQ_FALLBACK.to_string());
...
// unlike TorrentQQ. Category comes through as Korean text (e.g. "[
